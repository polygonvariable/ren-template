...
void FSeasonDebugWidget::DrawWidget(float DeltaTime)
{
	...
	{
		if (SlateIM::BeginViewportRoot(TEXT("SeasonWindow"), GEngine->GameViewport))
		{
			SlateIM::BeginTable();
			...
			if (Controller)
			{
				...
				if (SlateIM::NextTableCell())
				{
					...
					if (CollectionAsset)
					{
						...
						const USeasonAsset* SeasonAsset = CollectionAsset->GetSeasonByDay(Controller->GetEditorCurrentDay(), CollectionAsset->YearLength, Transition);
...
UWorld* FSeasonDebugWidget::GetWorld()
{
	if (GEngine)
	{
...
USeasonController* FSeasonDebugWidget::GetController()
{
	...
	USeasonSubsystem* Subsystem = USeasonSubsystem::Get(GetWorld());
	if (Subsystem)
	{
