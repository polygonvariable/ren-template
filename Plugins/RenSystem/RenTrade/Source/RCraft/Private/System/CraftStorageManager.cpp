...
const FCraftInstance* UCraftStorageManager::GetItem(const FTradeKey& TradeKey) const
{
	if (!IsValid(LocalStorage))
	{
		return nullptr;
	}
	const FCraftInstance* FoundData = LocalStorage->CraftItems.Find(TradeKey);
	return FoundData;
}
...
bool UCraftStorageManager::AddItem(const FTradeKey& TradeKey, FTimespan BatchProcessingTime)
{
	...
	TMap<FTradeKey, FCraftInstance>& CraftItems = LocalStorage->CraftItems;
	FCraftInstance* FoundData = CraftItems.Find(TradeKey);
	if (FoundData)
	{
	...
	{
		CraftItems.Add(TradeKey, FCraftInstance(1, FDateTime::Now(), BatchProcessingTime));
	}
