...
void UCraftCollectionUI::DisplayEntries()
{
	...
	CraftSubsystem->QueryItems(TradeAsset, TradeCollectionId, QuerySource,
		[this, TradeAssetId](const FPrimaryAssetId& ItemAssetId, const FTradeAssetDetail& ItemDetail, const FCraftInstance* CraftData) {
...
void UCraftCollectionUI::NativeConstruct()
{
	...
	if (IsValid(CraftSubsystem))
	{
		...
		{
			StorageManager->GetOnStorageUpdated().AddUObject(this, &UCraftCollectionUI::RefreshEntries);
		}
	}
