...
USubsystem* FSubsystemLibrary::GetSubsystemByInterface(UWorld* Context, UClass* InterfaceClass)
{
	if (!IsValid(Context))
	{
		return nullptr;
...
USubsystem* FSubsystemLibrary::GetSubsystemByInterface(UGameInstance* Context, UClass* InterfaceClass)
{
	if (!IsValid(Context))
	{
		return nullptr;
